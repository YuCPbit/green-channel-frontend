...
const filterStatus = ref('2') // default: pending college
...
<template>
  <div class="view-container">
    <header class="view-header">
      <div>
        <h2 class="view-title">学院审核</h2>
        <p class="subtitle">审核本学院学生的困难补助申请</p>
      </div>
    </header>
    ...
    <div class="table-card">
      <table class="data-table">
        <thead><tr><th>申请编号</th><th>学生</th><th>学号</th><th>批次</th><th>金额</th><th>发起</th><th>状态</th><th>时间</th><th>操作</th></tr></thead>
        <tbody>
          ...
          <tr v-for="a in applies" :key="a.id" v-else>
            <td>{{ a.applyNo }}</td><td>{{ a.studentName }}</td><td>{{ a.studentNo }}</td><td>{{ a.batchName }}</td><td>{{ formatMoney(a.applyAmount) }}</td>
            ...
            <td><button class="text-btn" @click="openDetail(a)">详情</button><button v-if="a.status===2" class="text-btn edit-btn" @click="openReview(a)">审核</button></td>
          </tr>
        </tbody>
      </table>
    </div>
    ...
    <div v-if="reviewTarget" class="modal-overlay" @click.self="reviewTarget=null">
      <div class="modal-card">
        ...
        <form @submit.prevent="submitReview">
          <label>审核动作<select v-model="reviewForm.action" required><option :value="1">通过</option><option :value="2">退回修改</option><option :value="3">不通过</option></select></label>
          <label>建议金额 (元)<input type="number" v-model="reviewForm.suggestAmount" min="0.01" step="0.01" /></label>
